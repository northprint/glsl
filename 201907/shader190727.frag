...
void main() {
    ...
    vec3 cDir = vec3(0.0, sin(time), 1.0);
    vec3 cUp  = vec3(0.0, 5.0, 0.0);
    ...
    if(abs(dist) < 0.001){
        ...
        color = diff + vec3(spec) * vec3(cos(halfLE.x + time),sin(halfLE.y + time),cos(halfLE.y + time));
    }
