...
float sdCrossCapsule(vec3 p, float r){
  ...
  vec3 p1 = p;
  vec3 p2 = p;
  p1.x -= clamp( p1.x, -2.0, 2.0 );
  p2.z -= clamp( p2.z, -2.0, 2.0 );
...
void main() {
    ...
    vec3 cPos = vec3(0.0, 0.0, 5.0 * time);
    ...
    if(abs(dist) < 0.001){
        ...
        color = diff + vec3(spec) * vec3(sin(halfLE.x + time),cos(halfLE.y + time),sin(halfLE.y + time));
    }
