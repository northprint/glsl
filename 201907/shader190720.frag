...
float distFunc(vec3 p){
    ...
    for(int i=0; i<4; i++) {
        ...
        st.yz *= rotate(time * 0.2);
        st.xy = pmod(st.xy, 6.0);
        st.y -= mix(6.0, 6.0, 6.0);
    }
    return sdCrossCapsule(st,1.0);
}
...
void main() {
    ...
    vec3 cPos = vec3(0.0, 5.0, 3.0);
    vec3 cDir = vec3(0.0, 0.0,-0.5);
    vec3 cUp  = vec3(0.0,  0.5, 0.0);
    ...
    if(abs(dist) < 0.001){
        ...
        color = diff + vec3(spec) * vec3(0.0,0.2,0.8);
    }
