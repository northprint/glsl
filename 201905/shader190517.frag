...
void main() {
    ...
    vec3 color = vec3(snoise(st + pattern(st, wave(1.0)) + time),
    snoise(st + pattern(st, wave(2.0)) + time),
    snoise(st + pattern(st, wave(3.0)) + time));
    color = mix(color,vec3(1.0, 0.0, 0.5), 0.5);
