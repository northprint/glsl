...
float cycle(vec2 p,float radius){
    ...
    return snoise(vec2(abs(sin(a) + r)));
}

float wave(vec2 st, float n) {
    st = max(st, n + (time * 0.05));
    float d = length(st);
    return dot(min(d,1.0),n);
}
...
void main() {
    ...
    vec3 cUp  = vec3(0.1, 0.0, 0.3);
    vec3 cSide = cross(cDir, cUp);
    float targetDepth = 0.3;
    // ray
    vec3 ray = normalize(cSide * st.x + cUp * st.y + cDir * targetDepth);

    vec3 color = vec3(1.0-cycle_wave(ray.xy, 12.0),cycle_wave(ray.xy, 24.0),cycle_wave(ray.xy, 36.0));
