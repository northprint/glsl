...
float wave(vec2 st, float n) {
    st = max(st, n + time * 0.2);
    float d = max(length(st), n);
    return dot(sin(d) * cos(d), 1.0) * 20.0;
}

float cycle_wave(vec2 st, float n){
    return length(st) * wave(st, n);
}
...
float tile(vec2 st, float n){
    ...
    for (int y= -1; y <= 1; y++) {
        for (int x= -1; x <= 1; x++) {
            ...
            // Animate the point
            point = 0.5 + 0.8 * sin(time * point);
            // Vector between the pixel and the point
            vec2 diff = neighbor + point - f_st;
            // Distance to the point
            float dist = length(cycle_wave(diff,n));
...
void main() {
    ...
    color = vec3(tile(st,2.0),tile(st,4.0),tile(st,6.0));
