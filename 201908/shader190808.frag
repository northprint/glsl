...
float distFunc(vec3 p){
    ...
    for(int i=0; i<8; i++) {
        q = abs(q) - 0.5; // Fold
        q.xz *= rotate(1.0 + (time * 0.3)); // 回転
        q.xy *= rotate(1.0 + (time * 0.2)); // 回転
    }

    //床
    float f = p.y += 10.0;
    return min(sdBox(q,1.0),f);
}
...
vec3 tex(vec2 st){
    ...
    vec3 color = palette(st.x + st.y, vec3(0.5), vec3(0.5), vec3(1.0), vec3(0.0, 0.33, 0.67));
    return color;
}
